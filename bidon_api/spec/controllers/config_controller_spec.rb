...
RSpec.describe ConfigController, type: :controller do
  ...
  context 'X-BidOn-Version header present' do
    ...
    context 'empty response' do
      ...
      it 'returns 200 with ok' do
        allow_any_instance_of(Config::Response).to receive(:present?).and_return(false)
    ...
    context 'invalid request' do
      ...
      it 'returns 422 with error' do
        allow_any_instance_of(Config::Request).to receive(:valid?).and_return(false)
    ...
    context 'error request' do
      ...
      it 'returns 500 with error' do
        allow(Config::Request).to receive(:new).and_raise(StandardError)
