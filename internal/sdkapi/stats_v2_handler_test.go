...
func SetupStatsV2Handler() sdkapi.StatsV2Handler {
	mockHandler := &mocks.StatsV2NotificationHandlerMock{}
...
func TestStatsV2Handler_Handle(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "valid request",
			requestPath:  "testdata/stats_v2/valid_request.json",
			expectedCode: http.StatusOK,
		},
		{
			name:         "invalid request",
			requestPath:  "testdata/stats_v2/invalid_request.json",
