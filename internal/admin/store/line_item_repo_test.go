...
func TestLineItemRepo_ListWithCursor(t *testing.T) {
	tx := testDB.Begin()
	defer tx.Rollback()
	...
	cursorAfter, _ := paginator.New(paginator.WithKeys("ID")).GetCursorEncoder().Encode(allItems[2])
	cursorBefore, _ := paginator.New(paginator.WithKeys("ID")).GetCursorEncoder().Encode(allItems[2])
	...
	testcases := []struct {
		...
		{
			name:    "limit is set",
			qParams: map[string][]string{"limit": {"2"}},
			want:    allItems[:2],
		},
		{
			name: "cursor_after is set",
			qParams: map[string][]string{
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			got, _, err := repo.ListWithCursor(context.Background(), tc.qParams)
			if err != nil {
				t.Fatalf("repo.List(ctx) = %v, %q; want %+v, %v", got, err, tc.want, nil)
			}
