...
func (f *AppDemandProfileFetcher) Fetch(ctx context.Context, appID int64, adapterKeys []adapter.Key) ([]config.AppDemandProfile, error) {
	...
	profiles := make([]config.AppDemandProfile, len(dbProfiles))
